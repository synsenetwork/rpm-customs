...
Packages currently tracked: `amneziawg-dkms`, `amneziawg-tools`, `claude-desktop`, `jetbrains-toolbox`, `mesa-git`, `qemu-synse`, `scrcpy`, `scx-scheds`, `scx-synse-manager`, `scx-tools`, `synse-settings`, `wooting-udev`, `wootility-beta`, `zed`.
...
`qemu-synse` is based on Fedora 44's QEMU 10.2.2 dist-git package and retains its subpackage layout, local sources, and Fedora patches. Its `Source0` is pinned to a commit from `synsenetwork/qemu` containing the maintainer's coherent hardware identity patch. Every binary package is renamed from `qemu[-<suffix>]` to `qemu-synse[-<suffix>]`, provides the original Fedora package name, and conflicts with that original package so Fedora QEMU updates cannot replace or coexist with this fork. Keep QEMU's `Epoch: 2`. GitHub commit archives are not QEMU release-signed, so do not restore the upstream tarball signature sources or `%prep` GPG verification unless the source is switched back to an official release tarball.
