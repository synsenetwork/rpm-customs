...
`gamescope` is a personal fork at `github.com/synsejse/gamescope-synse` and the spec is a slimmed-down adaptation of Fedora's `gamescope.spec` — same `BuildRequires` and the same pinned `reshade`/`vkroots` sources — only `Source0`, `URL`, `Version`/`Release`, and the `%setup -n` line differ. **No patches are applied in the spec**: Fedora's `Allow-to-use-system-wlroots`, `Use-system-stb-glm`, and the reshade `0001-cstdint` change are expected to be merged in the fork's source itself; the spec builds the source as-is. When upstream Fedora bumps `reshade_commit` or `vkroots_commit` we should sync those too.
